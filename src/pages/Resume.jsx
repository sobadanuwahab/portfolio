...
const about = {
  ...
  info: [
    { icon: <FaUser />, fieldName: "Name", fieldValue: "Soba Danu Wahab" },
    { icon: <FaPhone />, fieldName: "Phone", fieldValue: "(+62) 813 1433 3352" },
    { icon: <FaBriefcase />, fieldName: "Experience", fieldValue: "3+ Years" },
    { icon: <FaSkype />, fieldName: "Skype", fieldValue: "danu.14" },
    { icon: <FaGlobe />, fieldName: "Nationality", fieldValue: "Indonesian" },
    { icon: <FaEnvelope />, fieldName: "Email", fieldValue: "sdanuw@gmail.com" },
    { icon: <FaCode />, fieldName: "Freelance", fieldValue: "Available" },
    { icon: <FaGlobe />, fieldName: "Languages", fieldValue: "Bahasa, English" },
  ],
};
...
const experience = {
  ...
  items: [
    { company: "Tech Solution Inc.", position: "Full Stack Developer", duration: "2022 - Present", type: "Full Time", location: "Remote" },
    { company: "Web Design Studio", position: "Front-End Developer Intern", duration: "2020 - 2021", type: "Internship", location: "Hybrid" },
    { company: "E-commerce Startup", position: "Freelance Web Developer", duration: "2021 - 2022", type: "Freelance", location: "Remote" },
    { company: "Platform e-ticket event", position: "Front-End Developer Intern", duration: "2025 - 2026", type: "Internship", location: "On-site" },
    { company: "Digital Agency", position: "UI/UX Designer", duration: "2018 - 2019", type: "Full Time", location: "On-site" },
  ],
};
...
const education = {
  ...
  items: [
    { institution: "Online Course Platform", degree: "Full Stack Web Development Bootcamp", duration: "2022", grade: "Advanced" },
    { institution: "Codecademy", degree: "Front-End Track", duration: "2021", grade: "Professional" },
    { institution: "Online Course", degree: "Programming Course", duration: "2019 - 2020", grade: "Intermediate" },
    { institution: "Tech Institute", degree: "Certified Web Developer", duration: "2016 - 2018", grade: "Professional" },
    { institution: "University Sultan Ageng Tirtayasa", degree: "Electrical Engineering", duration: "2012 - 2016", grade: "Bachelor Degree" },
  ],
};
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <TabsContent value="experience" className="w-full mt-0">
                  <div className="flex flex-col gap-8">
                    <div className="space-y-6">
                      <Badge variant="outline" className="px-4 py-2 text-accent border-accent/30 bg-accent/10">
                        {experience.subtitle}
                      </Badge>
                    ...
                    <ScrollArea className="h-[450px] pr-4">
                      <motion.ul variants={containerVariants} initial="hidden" animate="visible" className="space-y-4">
                        {experience.items.map((item, index) => (
                          <motion.li key={index} variants={itemVariants} whileHover={{ scale: 1.02 }} className="group">
                                ...
                                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                                  <div className="space-y-3">
                                    <div className="flex items-center gap-3 flex-wrap">
                                      <span className="px-3 py-1 bg-accent/10 text-accent rounded-full text-sm font-medium">{item.duration}</span>
                                      <span className="px-3 py-1 bg-white/5 text-white/70 rounded-full text-sm">{item.type}</span>
                                      <span className="px-3 py-1 bg-white/5 text-white/70 rounded-full text-sm flex items-center gap-1">
                                        <FaMapMarkerAlt className="text-xs" />
                                        {item.location}
                                      </span>
                                    </div>
                                    <h4 className="text-xl font-semibold text-white group-hover:text-accent transition-colors">{item.position}</h4>
                ...
                <TabsContent value="education" className="w-full mt-0">
                  <div className="flex flex-col gap-8">
                    <div className="space-y-6">
                      <Badge variant="outline" className="px-4 py-2 text-accent border-accent/30 bg-accent/10">{education.subtitle}</Badge>
                      <h3 className="text-3xl xl:text-4xl font-bold bg-gradient-to-r from-white to-white/70 bg-clip-text text-transparent">{education.title}</h3>
                      <Description paragraphs={education.description} />
                    </div>
                    <ScrollArea className="h-[450px] pr-4">
                      <motion.ul variants={containerVariants} initial="hidden" animate="visible" className="space-y-4">
                        {education.items.map((item, index) => (
                          <motion.li key={index} variants={itemVariants} whileHover={{ scale: 1.02 }}>
                            <Card className="bg-gradient-to-br from-[#232329] to-[#1A1A1F] border-none hover:shadow-lg hover:shadow-accent/5 transition-all duration-300">
                              <CardContent className="p-6">
                                <div className="space-y-3">
                                  <div className="flex items-center gap-3 flex-wrap">
                                    <span className="px-3 py-1 bg-accent/10 text-accent rounded-full text-sm font-medium">{item.duration}</span>
                                    <Badge variant="secondary" className="bg-white/5 text-white/70 hover:bg-white/10">{item.grade}</Badge>
                                  </div>
                ...
                <TabsContent value="skills" className="w-full mt-0">
                  <div className="flex flex-col gap-8">
                    <div className="space-y-6">
                      <Badge variant="outline" className="px-4 py-2 text-accent border-accent/30 bg-accent/10">{skills.subtitle}</Badge>
                      <h3 className="text-3xl xl:text-4xl font-bold bg-gradient-to-r from-white to-white/70 bg-clip-text text-transparent">{skills.title}</h3>
                      ...
                      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-8">
                        {skills.categories.map((category, idx) => (
                          <motion.div key={idx} variants={itemVariants} className="space-y-4">
                            <h4 className="text-lg font-semibold text-white/90 border-l-4 border-accent pl-3">{category.name}</h4>
                              ...
                              {category.skills.map((skill, index) => (
                                <TooltipProvider key={index} delayDuration={100}>
                                  <Tooltip>
                                      ...
                                      <motion.div whileHover={{ y: -5 }} className="group relative">
                                        <Card className="bg-gradient-to-br from-[#232329] to-[#1A1A1F] border-none hover:bg-accent/5 transition-all duration-300 cursor-pointer">
                                          <CardContent className="p-6 flex flex-col items-center gap-3">
                                            ...
                                            <span className="text-sm font-medium capitalize text-white/70 group-hover:text-accent">{skill.name}</span>
                                    ...
                                    <TooltipContent side="top" className="bg-[#2A2A33] border-white/10 text-white">
                                      <p className="font-medium">{skill.name}</p>
                                      <p className="text-xs text-white/60">Proficiency: {skill.level}%</p>
                                    </TooltipContent>
                                  </Tooltip>
                                </TooltipProvider>
                              ))}
                ...
                <TabsContent value="about" className="w-full mt-0">
                  <div className="flex flex-col gap-8">
                    <div className="space-y-6">
                      <Badge variant="outline" className="px-4 py-2 text-accent border-accent/30 bg-accent/10">Get to know me</Badge>
                      <h3 className="text-3xl xl:text-4xl font-bold bg-gradient-to-r from-white to-white/70 bg-clip-text text-transparent">{about.title}</h3>
                      <Description paragraphs={about.description} />
                    </div>
                    <Card className="bg-gradient-to-br from-[#232329] to-[#1A1A1F] border-none">
                      <CardContent className="p-8">
                        <motion.ul variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          {about.info.map((item, index) => (
                            ...
                            >
                              <div className="text-2xl bg-accent/10 p-3 rounded-lg text-accent">{item.icon}</div>
                              <div className="flex flex-col">
                                <span className="text-sm text-white/50">{item.fieldName}</span>
                                <span className="font-medium text-white">{item.fieldValue}</span>
                              </div>
                            </motion.li>
                          ))}
                        </motion.ul>
